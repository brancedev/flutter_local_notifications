...
public class FlutterLocalNotificationsPlugin
  ...
  private static void scheduleNotification(
    ...
    safelySchedule(context, notificationDetails, pendingIntent);
  ...
  /**
   * Tries to schedule exact notification if we have the permission, if not, we
   * schedule a non exact notification
   */
  private static void safelySchedule(Context context, NotificationDetails notificationDetails,
      PendingIntent pendingIntent) {
    AlarmManager alarmManager = getAlarmManager(context);

    boolean allowedExact = true;
    if (VERSION.SDK_INT >= VERSION_CODES.S) {
      allowedExact = alarmManager.canScheduleExactAlarms();
    }

    if (allowedExact) {
      scheduleExact(notificationDetails, pendingIntent, alarmManager);
    } else {
      scheduleNotExact(notificationDetails, pendingIntent, alarmManager);
    }
  }

  private static void scheduleNotExact(NotificationDetails notificationDetails, PendingIntent pendingIntent,
  ...
  private static void scheduleExact(NotificationDetails notificationDetails, PendingIntent pendingIntent,
      AlarmManager alarmManager) {
    if (BooleanUtils.getValue(notificationDetails.allowWhileIdle)) {
